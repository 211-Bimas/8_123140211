...
fun App(
    ...
    // 1. BUNGKUS APLIKASI DENGAN KoinApplication
    KoinApplication(application = {
        modules(
            // Module dinamis untuk dependency bawaan platform (Database & Settings)
            module {
                single { NotesDatabase(driverFactory.createDriver()) }
                single { settings }
            },
            // Module utama yang ada di AppModule.kt
            sharedModule
        )
    }) {
        // 2. TINGGAL PANGGIL koinInject() UNTUK VIEWMODEL, GAK PERLU MANUAL LAGI!
